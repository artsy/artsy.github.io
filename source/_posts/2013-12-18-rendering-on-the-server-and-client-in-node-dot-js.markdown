...
Today we're happy to announce [Ezel](http://ezeljs.com/), our open source boilperplate we use to boostrap our Node projects and the [various](https://github.com/artsy/benv) [node](https://github.com/artsy/backbone-super-sync) [modules](https://github.com/artsy/sharify) that built up to it.

In his article, [_Isomorphic JavaScript: The Future of Web Apps_](http://nerds.airbnb.com/isomorphic-JavaScript-future-web-apps/), Spike Brehm from AirBnB describes this growing trend well and we're excited to be a part of it. In this article I'll tell Artsy's story of moving from a single monolithic Rails application to modular [Backbone](http://backbonejs.org/) apps that run in Node and the browser.
...
* SEO issues like building [escaped fragment](https://developers.google.com/webmasters/ajax-crawling/docs/specification) pages in Rails while our users saw what Backbone rendered.
...
A monolithic Rails app that treats it's client-side code as a second class citizen was clearly not going to scale. Our poor mobile web experience was a good candidate to try something new. So we started building a separate mobile optimized website at m.artsy.net.
...
We open sourced this combination of tools and patterns into [Ezel](http://ezeljs.com/). Ezel is a light-weight boilerplate project using [Express](http://expressjs.com/) and [Backbone](http://backbonejs.org/) for structure, and [Browserify](http://browserify.org/) to compose modules where some can be shared server/client.
...
With templates and models require-able server/client, sharing rendering code became much simpler. An example using the same artwork model and detail template server/client might look like this:
...
Not only does sharing code server/client let you easily optimize page rendering for fast page loads, but development becomes a lot nicer because we can re-use server-side JavaScript tools including...

### Package Mangers

With Browserify we were able to use npm as a package manager for server, or client-side, dependencies. There are [other](http://bower.io/) [package](http://component.io/) [managers](http://jamjs.org/) for the client-side. But since we were already using npm, and npm supports git urls, we could usually point to the project hosted on npm or Github without having to fork it.
...
Testing is light-years ahead because you can test all of your code headless in Node. I wrote [an entire article](/blog/2013/06/14/writing-headless-backbone-tests-with-node-dot-js/) on this, and with Browserify it's even better.
...
We wanted to avoid a monolithic organization that groups code by type such as "stylesheets", "javascripts", "controllers", etc. Not only is this a maintenance problem as it makes boundaries of your app unclear, but it also affects your users because it encourages grouping all of your assets into large monolithic packages that take a long time to download.
...
It's also worth noting: To avoid CSS spaghetti we followed a simple convention of namespacing all of our classes/ids by the app or component name it was a part of. This was inspired by a [blog post from Philip Walton](http://philipwalton.com/articles/css-architecture/).
