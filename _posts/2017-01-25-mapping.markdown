...
I have a bunch of scripts like this, one which generate more CSV files
...
Once this was ready I did another script to pull things from another API
...
CSV.foreach("/Users/orta/Downloads/end-result-2.csv") do |row|
  if row[1] && row[1].strip.length > 0 && row[2] and row[3]
    data << { name: row[0], radius: 4, created_at: row[1].to_i, latitude: row[2].to_f, longitude: row[3].to_f }
  end
end
